...
const moment = require('moment') // Para trabalhar com data

const session = require('telegraf/session') // É obrigatório o uso da sessão ao trabalhar com scena e stage
...
// Formata a data para o padrão BR, e atribui uma string vazia caso não a receba
const formatarData = data =>
    data ? moment(data).format('DD/MM/YYYY') : ''

// Exibe a tarefa formatada para apresentação
...
// Retorna um botão inline com apenas uma data e descrição, que contem uma ação de mostrar
// que ao clicada detalha a tarefa
...
// Cria botões inline, que permite realizar ações com determinada tarefa
...
bot.command('dia', async ctx => {
    const tarefas = await getAgenda(moment()) // O moment() sem nenhum parametro passa a data atual
...
// Pega o texto digitado e cria uma tarefa, dps de criado a exibe
